...
const NKFieldBuilder = <T,>({ fields, title, record, extra, isFetching, containerClassName }: FieldBuilderProps<T>) => {
    return (
        <div className={clsx('fade-in flex flex-col gap-4  rounded-lg    bg-white px-8 py-8', containerClassName)}>
            {isFetching ? (
                <div>Loading...</div>
            ) : (
                <>
